...
!-->
					<div class="calc-subtotal-list totals" style="margin-top: 20px; padding-top: 10px;" ref="calcTotals" :class="{'unit-enable': showUnitInSummary}" v-show="!summaryDisplay || showAfterSubmit">
						...
						<template v-for="item in formulaConst">
							...
							{{ item.discount }}
							<cost-total v-else-if="(item.total > 0 && !item.label.includes('Annual')) || item.label.includes('KOSTNAD')" :value="item.total" :discount="item.discount" :field="item.data" :id="calc_data.id" @condition-apply="renderCondition"></cost-total>
